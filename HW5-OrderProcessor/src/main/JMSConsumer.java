...
public class JMSConsumer implements MessageListener {
    ...
    public void onMessage(Message msg) {
        try {
            ...
            String[] msgArr = {msgText};
            if(msgText.equals("Booking")){
            	JMSProducerBookings.main(msgArr);
            }else if(msgText.equals("Trip")){
            	JMSProducerTrips.main(msgArr);
            }
    ...
    public void receive(String queueName) throws Exception {
        ...
        System.out.println("Connected to " + queue.toString() + ", waiting confirmation...");
        try {
            synchronized (this) {
                while (true) {
                    this.wait();
